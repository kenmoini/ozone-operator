...
# CONTAINER_BUILD_TOOL is the container tool to use to build the containers
...
# operator.o3/ozone-operator-bundle:$VERSION and operator.o3/ozone-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= kenmoini/ozone-operator
...
IMG ?= quay.io/kenmoini/ozone-operator
...
.PHONY: regen-source
regen-source: build generate manifests
##@ General
...
$(KUSTOMIZE): $(LOCALBIN)
	if [ ! -f $(LOCALBIN)/kustomize ]; then curl -s $(KUSTOMIZE_INSTALL_SCRIPT) | bash -s -- $(subst v,,$(KUSTOMIZE_VERSION)) $(LOCALBIN); fi
